...
import {
  awardPetXp, clearStageFor, coupleVitals, ensureIdentity, gardenMomentum,
  loadWorldProgress, travelToIsland,
} from '../../db/repository';
...
export function EveGardenPage() {
  ...
  useEffect(() => {
    if (!sprite || !host.current) return undefined;
    let live = true;
    ...
    import('./scene/game')
      .then(({ startGarden }) => {
        ...
        scene.current = startGarden(
          host.current,
          { island, stage, monsterSprite: sprite, hour: new Date().getHours(), dark },
