...
/**
 * Turning a charge-strip tap into the row the rest of the app already reads.
 *
 ...
 *
 * The strip cannot ask for detail — it is one chip beside a fight — so each of
 * these writes the smallest honest row and leaves the real page to refine it. A mood logged here is the neutral reading, which is what
...
export async function logActivity(
  ...
  switch (activity) {
    case 'Mood': {
      ...
      // sat down and set, with a neutral default, because they tapped a charge
    ...
    // Rest, Gratitude and Nourish have no table of their own. They are real
    // charges and real XP, and they deliberately write nothing: inventing a
    // `rest` table so a chip has somewhere to land would be a schema change in
    // service of a UI affordance. The XP award is the record, and it is what
    // `todaysCharges` reads back.
    case 'Rest':
    case 'Gratitude':
    case 'Nourish':
      return;
