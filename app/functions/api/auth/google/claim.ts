...
import { CLAIM_CONSUME_SQL, sweep } from '../_oauth';
import type { GoogleEnv } from '../_google';

/**
 * Exchange a one-time claim code for its result.
 *
 * This is where a recovery actually takes effect, and doing it here rather than
 * in the callback is deliberate. The obvious design has the callback rotate the
 * bearer and park it somewhere for the app to collect, which leaves a live
 * plaintext token at rest for the length of the claim window. Minting it here
 * means the new token is created, hashed into `members`, and returned in a
 * single response, and never exists anywhere at rest at all.
 *
 ...
 * necessarily signs the old one out. That is the correct behaviour for the case
 * this exists for (a lost or replaced phone), and it is also the safety
...
export const onRequestPost: PagesFunction<GoogleEnv> = async ({ request, env }) => {
  ...
  if (claim.outcome === 'linked') {
    ...
    // learning even that the connection was made.
